<template>
     <div>
        <h1 class= "text-center pt-3"> Vestuario Mujer</h1>
        <v-divider></v-divider>   
 <div class="card-container mt-3">
      <div class="card" v-for="(item, index) in womenClothData" :key="index">
...
export default {
    ...
    data: function(){
     ...
     computed: {},
    methods: {
        async    fetchWomenCloth() {
     try {
      let response = await  fetch("/ropaMujer.json")
      let json = await response.json();         
        this.womenClothData = json; 
    }
     catch (error) {
     console.log(error);
     }
